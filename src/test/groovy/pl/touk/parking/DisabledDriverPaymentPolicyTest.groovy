...
class DisabledDriverPaymentPolicyTest extends Specification {
    ...
    def 'disabled driver pays nothing after the first started hour'() {
        given:
        parkingUsageService.startParkingMeter(disabledDriverOfVehicleWith(vehicleNumber()), at16_00)
    ...
    def 'disabled driver pays 2 PLN after the second started hour'() {
        given:
        parkingUsageService.startParkingMeter(disabledDriverOfVehicleWith(vehicleNumber()), at16_00)
    ...
    def 'disabled driver pays 1.2 times more than the hour before for 3rd and each next hour'() {
        setup:
        parkingUsageService.startParkingMeter(disabledDriverOfVehicleWith(vehicleNumber()), at16_00)
        ...
        where:
        endTime                  | payment
        LocalTime.of(18, 0, 0)   | Money.of(2 * 1.2)
        LocalTime.of(18, 59, 59) | Money.of(2 * 1.2)
        LocalTime.of(19, 59, 59) | Money.of(2 * 1.2 * 1.2)
        LocalTime.of(20, 59, 59) | Money.of(2 * 1.2 * 1.2 * 1.2)
    }

    private Driver disabledDriverOfVehicleWith(String vehicleNumber) {
        return new Driver(vehicleNumber, new DisabledDriverPaymentPolicy())
    }
